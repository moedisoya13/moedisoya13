...
def main() -> int:
    ...
    ap.add_argument(
        ...
        default=None,
        help="Force a specific Whisper backend. 'local' runs faster-whisper on this "
             "machine (no audio leaves it). Default: WATCH_WHISPER from config, else "
    ...
    if not transcript_segments and not args.no_whisper and video_path and meta.get("has_audio"):
        whisper_pref = args.whisper or config.get("whisper")
        if whisper_pref == "local":
            import local_whisper  # noqa: E402  (optional dep — imported only when selected)
            if not local_whisper.is_available():
                print(
                    "[watch] local Whisper selected (WATCH_WHISPER=local) but faster-whisper "
                    "is not installed — run: python -m pip install --user faster-whisper",
                    file=sys.stderr,
                )
            else:
                try:
                    all_segments, used_backend = local_whisper.transcribe_local(
                        video_path,
                        work / "audio.mp3",
            ...
            if backend and api_key:
                ...
                hint = (
                    f"--whisper {whisper_pref} was set but the matching API key is missing"
                    if whisper_pref else
