...
def _whisper_mode() -> str | None:
    """Selected Whisper backend from env/.env: None (auto API) | groq | openai | local."""
    mode = (_read_env_key("WATCH_WHISPER") or "").strip().lower()
    return mode if mode in {"groq", "openai", "local"} else None
...
def _status() -> dict:
    ...
    mode = _whisper_mode()
    # "has_transcription" = this install can produce a transcript when captions
    # are missing. For local mode that means faster-whisper is importable; for
    # the API modes it means a key is set.
    if mode == "local":
        has_transcription = _faster_whisper_installed()
        backend = "local" if has_transcription else None
    else:
        has_transcription, backend = _have_api_key()
...
def cmd_check() -> int:
    ...
    if not s["has_api_key"] and not s["setup_complete"]:
        if s.get("whisper_mode") == "local":
            parts.append("faster-whisper not installed (WATCH_WHISPER=local)")
        else:
...
def cmd_install() -> int:
    ...
    # Local Whisper mode: transcription runs on this machine, so no API key is
    # needed. Ensure faster-whisper is importable (pip-install it if not).
    if _whisper_mode() == "local":
        if not _faster_whisper_installed():
            print("[setup] local Whisper selected (WATCH_WHISPER=local) — installing faster-whisper...", file=sys.stderr)
            rc = subprocess.run(
                [sys.executable, "-m", "pip", "install", "--user", "faster-whisper"]
            ).returncode
            if rc != 0 or not _faster_whisper_installed():
                print(
                    "[setup] could not install faster-whisper automatically. "
                    "Install it manually: python -m pip install --user faster-whisper",
                    file=sys.stderr,
                )
                return 3
        _write_setup_complete()
        print("[setup] ready. transcription: local whisper (faster-whisper, no audio leaves this machine).")
        return 0
...
def main() -> int:
    ...
    # Windows) that can't encode the em-dashes/arrows in our help text, which
    # would crash an otherwise-successful install. Force UTF-8 where supported.
