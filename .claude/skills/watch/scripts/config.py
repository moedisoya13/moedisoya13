...
# Whisper backend selection. None = auto-detect API keys (Groq then OpenAI);
# "local" runs faster-whisper on this machine so no audio leaves it.
WHISPERS = {"groq", "openai", "local"}
...
def get_config() -> dict[str, object]:
    ...
    if whisper is not None and whisper not in WHISPERS:
        whisper = None

    whisper_model = (
        os.environ.get("WATCH_WHISPER_MODEL")
        or file_values.get("WATCH_WHISPER_MODEL")
