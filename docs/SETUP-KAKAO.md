...
## 2-1. 링크 도메인 등록 (빠뜨리면 링크가 안 열립니다)

카카오는 메시지 템플릿의 링크를 **앱에 등록된 도메인으로만** 열어줍니다(링크 변조 방지).
등록하지 않으면 메시지는 정상 도착하지만 **항목을 눌러도 아무 반응이 없습니다.**

**[제품 설정] → [링크 관리]** 의 **웹 도메인**에 아래를 등록하세요.

```
https://news.hada.io
```

- 콘솔 버전에 따라 **[앱 설정] → [플랫폼] → Web → 사이트 도메인** 에 있기도 합니다.
  둘 다 보이면 양쪽 모두 등록해 두세요.
- ⚠️ **[앱 키] → [JavaScript SDK 도메인]** 과는 **다른 항목**입니다.
  거기 등록해도 메시지 링크는 열리지 않습니다. 가장 흔한 착오입니다.
- 최대 10개까지 등록할 수 있습니다.
- **저장 후 새로 보내는 메시지부터** 적용됩니다. 이미 받은 메시지의 링크는 그대로 안 열립니다.
...
3. **각 항목을 눌러 긱뉴스 원문이 열리는지 확인** — 눌러도 반응이 없으면 메시지가 잘못된 게
   아니라 2-1 의 도메인 등록이 빠진 것입니다. 등록 후 다시 보내면 열립니다.
