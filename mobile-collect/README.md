...
인수인계 문서의 방식대로 **추정과 실측을 구분해** 적습니다.

| 날짜 | 확인한 것 | 결과 |
|---|---|---|
| 2026-08-01 | iPhone Safari로 `m.coupang.com/vm/products/8868816986` 열기 | **정상 렌더** (실측, 스크린샷) |

관측된 것:

- 브레드크럼·상품 이미지·만족도까지 실제 콘텐츠가 나옵니다. 차단 페이지가 아닙니다.
- **로그아웃 상태에서 나옵니다.** 상단에 `로그인 / 회원가입`이 떠 있고 장바구니는 0입니다.
  수집에 로그인 세션이 필요 없다는 뜻이므로, §5가 경고한 계정 제재 위험을 아예 만들지 않습니다.
  **로그인한 채로 수집하지 마세요.**

아직 확인되지 않은 것 (첫 실물 캡처로만 알 수 있음):

- 옵션·가격·다른 판매자 구간이 이 화면 **아래**에 실제로 존재하는지
- 그 구간의 문구가 `parse.py`의 마커와 맞는지 → `pc-side/NOTES.md` 확인 4
