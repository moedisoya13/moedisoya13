...
### 회차가 통째로 버려지는 경우 — 백업 스케줄

`schedule` 은 지연되기만 하는 것이 아니라, 러너 부하가 높으면 **그 회차가 통째로 버려집니다.**
버려져도 재시도가 없고 실패한 job 조차 남지 않아서, 그날 다이제스트가 아무 흔적 없이
사라집니다(실제로 2026-09-08 1차가 이렇게 안 떴습니다).

그래서 40분 뒤 **KST 09:20** 에 백업 스케줄을 하나 더 걸어 두었습니다.
두 회차가 다 떠도 메시지는 한 번만 갑니다 — `geeknews.main` 이 오늘자
`digests/YYYY-MM-DD.md` 가 이미 있으면 수집·요약·발송을 시작하지도 않고 끝냅니다.
아카이브 파일은 발송에 성공한 뒤에만 쓰이고 곧바로 커밋되므로, 파일의 존재가 곧
'오늘 몫은 이미 갔다'는 뜻입니다.

`state/seen.json` 으로는 이 역할을 대신할 수 없습니다. 그쪽은 **같은 글**만 막을 뿐이어서,
1차 이후 피드에 새 글이 올라와 있으면 백업 실행이 그 새 글들로 두 번째 묶음을 만들어
그대로 보내 버립니다.
